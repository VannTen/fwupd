...
#define FU_IGSC_DEVICE_MEI_IGSCIF	 "a50cd987-9534-5945-8105-3fbfa37b8b79"
...
fu_igsc_device_get_subsystem_ids(FuIgscDevice *self, GError **error)
{
	...
	/* in older versions this command wasn't supported */
	if (!fu_igsc_device_heci_validate_response_header(self,
							  &res.response,
							  req.header.command_id,
							  &error_local)) {
		g_debug("ignoring HECI message response: %s", error_local->message);
		return TRUE;
	}
...
fu_igsc_device_open(FuDevice *device, GError **error)
{
	...
	if (!FU_DEVICE_CLASS(fu_igsc_device_parent_class)->open(device, error))
		return FALSE;
	return fu_mei_device_connect(FU_MEI_DEVICE(device), FU_IGSC_DEVICE_MEI_IGSCIF, 0, error);
}
...
fu_igsc_device_setup(FuDevice *device, GError **error)
{
	...
	version = g_strdup_printf("%04d.%04d", fw_code_version.hotfix, fw_code_version.build);
...
fu_igsc_device_get_fw_status(FuIgscDevice *self, guint line, guint32 *fw_status, GError **error)
{
	...
	tmp = fu_udev_device_get_sysfs_attr(FU_UDEV_DEVICE(self), "fw_status", error);
	if (tmp == NULL) {
		g_prefix_error(error, "device is corrupted: ");
		return FALSE;
	}
	lines = g_strsplit(tmp, "\n", -1);
	if (line > g_strv_length(lines)) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_NOT_SUPPORTED,
			    "line %u of %s is not possible",
			    line,
			    tmp);
		return FALSE;
	}
	hex = g_strdup_printf("0x%s", lines[line - 1]);
	if (!fu_strtoull(hex, &tmp64, 0x1, G_MAXUINT32 - 0x1, error)) {
		g_prefix_error(error, "fw_status %s is invalid: ", lines[line - 1]);
		return FALSE;
	}
	if (fw_status != NULL)
		*fw_status = tmp64;
...
{
	return fu_mei_device_connect(FU_MEI_DEVICE(self), FU_IGSC_DEVICE_MEI_IGSCIF, 0, error);
}
