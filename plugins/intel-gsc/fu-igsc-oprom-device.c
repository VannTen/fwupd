...
fu_igsc_oprom_device_probe(FuDevice *device, GError **error)
{
	...
	if (self->payload_type == GSC_FWU_HECI_PAYLOAD_TYPE_OPROM_CODE) {
		...
		fu_device_set_name(FU_DEVICE(self), "Igsc OPROM Code");
		...
		fu_device_set_name(FU_DEVICE(self), "Igsc OPROM Data");
...
fu_igsc_oprom_device_setup(FuDevice *device, GError **error)
{
	...
	/* get version */
	if (!fu_igsc_device_get_version_raw(
		igsc_parent,
		GSC_FWU_HECI_PART_VERSION_OPROM_CODE, // FIXME make property?
		(guint8 *)&oprom_ver,
		sizeof(oprom_ver),
		error)) {
	...
	version = g_strdup_printf("%04d.%04d.%04d.%04d",
				  oprom_ver.major,
				  oprom_ver.minor,
				  oprom_ver.hotfix,
				  oprom_ver.build);
